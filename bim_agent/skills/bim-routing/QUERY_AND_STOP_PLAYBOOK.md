...
- Use a second query only to test a material exclusion, counterexample, identity mapping, or source disagreement that the certifying query cannot resolve.
- Use specialized geometry or graph tools only when the planned metric or relationship requires them. Do not substitute bounding extents for path metrics or reference presence for graph reachability.
